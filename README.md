...
> **项目进展说明**
> 本项目已完成了核心策略的流程搭建（股票筛选 → 数据获取 → 信号生成），并验证了策略逻辑的可行性。**当前正在进行的下一步工作**是：
> 1.  **开发回测模块**，计算夏普比率、最大回撤等关键绩效指标。
> 2.  **参数优化与敏感性分析**，测试不同阈值对策略表现的影响。
> 3.  **扩展股票池**，将策略应用到更多行业。
...
- 对中信证券 vs 申万宏源进行完整流程，生成 Z-Score 图表和交易信号。
 ![中信证券 vs 申万宏源 价差分析与交易信号](pair1_spread_analysis.png)
